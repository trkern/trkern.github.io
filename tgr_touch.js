...
function tgr_grapher(id, grapher_objs, plotdata) {
	if (! (id in tgr_fingerbox)) {tgr_fingerbox[id] = [];}
	plotdata = plotdata || {};
...
var tgr_fingerbox = {};
var tgr_fingerdistance = -1;

function tgr_mdown(id,e) {
	tgr_fingerbox[id][e.pointerId] = e;
	var rect=e.target.getBoundingClientRect();
...
function tgr_mmove(id,e) {
	tgr_fingerbox[id][e.pointerId] = e;
	var l = Object.keys(tgr_fingerbox[id]);
	if (l.length == 2) {
		var d = Math.sqrt(Math.pow(tgr_fingerbox[i][0].clientX - tgr_fingerbox[i][1].clientX,2) + Math.pow(tgr_fingerbox[i][0].clientY - tgr_fingerbox[i][1].clientY,2));
		if (d < tgr_fingerdistance) {
			tgr_zoom_amt(id,d/tgr_fingerdistance);
			}
		tgr_fingerdistance = d;
		}
	else if (tgr_graph_array[id].plotdata.dragging) {
		var pd = tgr_graph_array[id].plotdata;
...
function tgr_mup(id,e) {
	delete tgr_fingerbox[id][e.pointerId];
	tgr_graph_array[id].plotdata.dragging = false;
	}

function tgr_zoom_amt(id,amt) {
	var pd = tgr_graph_array[id].plotdata;
	var xc = (pd.xmax + pd.xmin)/2;
	var xr = (pd.xmax - pd.xmin)/2;
	pd.xmax = xc + amt*xr;
	pd.xmin = xc - amt*xr;
	var yc = (pd.ymax + pd.ymin)/2;
	var yr = (pd.ymax - pd.ymin)/2;
	pd.ymax = yc + amt*yr;
	pd.ymin = yc - amt*yr;
	if (pd.no_y) {
		pd.ymin = pd.oymin;
...
function tgr_string(id, grapher_objs, plotdata) {
	...
	cstr += " onwheel='tgr_wheel("+'"'+id+'"'+",event)' onpointerdown='tgr_mdown("+'"'+id+'"'+",event)' onpointermove='tgr_mmove("+'"'+id+'"'+",event)' onpointerup='tgr_mup("+'"'+id+'"'+",event)' onpointerleave='tgr_mup("+'"'+id+'"'+",event)'></canvas>";
