...
function tkas_parse_find_frac(L,fz) {
	var s = tkas_parse_project(L);
	var re = /RSS/;
	var m = tkas_forward_match(re,s,fz);
	if (m) {return(tkas_fz(m));}
...
function tkas_pemdas_frac(L,fz) {
	var pf = tkas_parse_find_frac(L,fz);
	if (pf) {
		var zoom = tkas_parse_focus(L,pf);
		var newT = ["S",{op:"DIV",subs:[zoom[1][1],zoom[2][1]]}];
		return(tkas_parse_merge(L,pf,newT));
		}
	return(false);
	}
